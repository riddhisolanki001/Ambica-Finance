	...
	if gl_map:
		if not cancel:
			...
			if gl_map and len(gl_map) > 1:
				...
				save_entries(gl_map, adv_adj, update_outstanding, from_repost,doc_status)
			...
			make_reverse_gl_entries(gl_map, doc_status,adv_adj=adv_adj, update_outstanding=update_outstanding)
...
def save_entries(gl_map, adv_adj, update_outstanding,doc_status,from_repost=False):
	if not from_repost:
	...
	for entry in gl_map:
		make_entry(entry, adv_adj, update_outstanding, from_repost,doc_status)


def make_entry(args, adv_adj, update_outstanding, doc_status,from_repost=False):
	if doc_status==1:
		...
		# gle.custom_doc_status=doc_status
		gle.submit()
	else:
		gle = frappe.new_doc("Not Approve GL Entery")
		gle.update(args)
		gle.flags.ignore_permissions = 1
		gle.flags.from_repost = from_repost
		gle.flags.adv_adj = adv_adj
	...
	if gl_entries:
		...
		for entry in gl_entries:
			...
			if new_gle["debit"] or new_gle["credit"]:
				make_entry(new_gle, adv_adj, doc_status,"Yes")
